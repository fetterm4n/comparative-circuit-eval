...
\maketitle
\begin{abstract}
Standard accuracy evaluation of security fine-tuned LLMs does not test robustness to alias substitution — replacing a command name with an equivalent short form that preserves all malicious behavior. We present a pre-deployment method for identifying which capability families in a security fine-tuned model are vulnerable to this class of attack, using only canonical clean inputs and no evasion variant generation. The method has two steps: a linear probe at the base model’s classification boundary flags families where the inherited circuit representation has drifted after fine-tuning, and an indicator-token ablation test reveals whether specific command-name tokens now suppress rather than drive malicious classification in the fine-tuned model. A family showing suppressor behavior is specifically at risk from alias substitution: removing the full command name removes the suppressor and disrupts the payload-context signal the model has come to rely on instead.

We validate the method on Foundation-Sec-8B-Instruct, a cybersecurity fine-tune of Llama-3.1-8B-Instruct, evaluated on a 293-pair matched cohort of malicious and benign PowerShell scripts. Using causal interventions, we localize the classification circuit to a small set of late attention heads (Layer 12) inherited from Llama. A linear probe from base-model activations at the circuit boundary achieves \(r = 0.80\)--\(0.87\) per head, confirming the inherited representation is linearly readable. The indicator-token ablation test then identifies \texttt{Invoke-WebRequest} as the family where Foundation-Sec’s ablation delta diverges most clearly from Llama’s: Foundation-Sec shows a large positive delta (\(+1.13\) mean, positive in 73.7\% of scripts) while Llama shows a strong negative delta (\(-1.60\)). In the evasion benchmark, \texttt{Invoke-WebRequest} is the family that fails consistently on alias substitution (\texttt{iwr}), while Llama has zero misses on the same variants. The sign divergence, observable on canonical inputs before any evasion is attempted, correctly identifies the vulnerable family.

Prompt-level mitigations do not cleanly fix the problem: instructing Foundation-Sec to assess overall script purpose rather than individual constructs resolves the \texttt{Invoke-WebRequest} misses but simultaneously opens new misses on \texttt{Invoke-Expression} obfuscation. This motivates pre-deployment family-level testing over post-deployment prompt engineering.
\end{abstract}
