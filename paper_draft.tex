...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em,topsep=0pt,parsep=0pt,partopsep=0pt]
  ...
  \item \textbf{Activation-space monitoring is practical and cheap at family level:} A linear probe from base-model activations at the circuit boundary predicts each head's contribution at \(r = 0.81\)--\(0.87\), including \texttt{L12H15} at \(r = 0.87\). Cross-model transfer to Foundation-Sec ablation dependence reaches \(r = 0.41\), making per-script prioritization noisy but supporting family-level red-teaming when averaged across canonical examples.
\end{itemize}
...
\caption{Mean logit-diff delta when indicator tokens are mean-ablated, broken down by indicator family and model. A negative delta means removing the indicator tokens \emph{reduces} malicious confidence (the token was acting as a driver). A positive delta means removing the tokens \emph{increases} malicious confidence (the token was acting as a suppressor). \texttt{Invoke-WebRequest} is the only family where Foundation-Sec (orange) shows a positive delta (\(+1.13\)), while Llama (blue) shows the expected negative delta (\(-1.60\)). This sign inversion identifies \texttt{Invoke-WebRequest} as the family where alias substitution (\texttt{iwr}) produces consistent classification failures in the current benchmark. The DownloadFile Llama bar extends below the axis limit (annotated value: \(-8.0\)); the y-axis is capped at \(-5.5\) to preserve readability of the smaller-delta families.}
