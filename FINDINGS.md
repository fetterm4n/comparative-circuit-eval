...
- The evasion benchmark now supports that late-stage claim across more than one family and more than one outcome type. In the first candidate sweep, `downloadstring_psobject_invoke` produced `2/6` misses, both inside the `FromBase64String` family. In the updated strict candidate sweep, `invoke_webrequest_alias` still produces `4/4` misses on the reviewed `Invoke-WebRequest` slice and remains the clearest failure mode in the repo. By contrast, the newly benchmarked strict candidate slices for `DownloadFile` and `-EncodedCommand` produce `0/4` and `0/2` misses respectively, and the separate provisional `IEX` extension also produces `0/4` misses. A second more realistic robustness benchmark now adds string-reconstruction techniques such as backticks, format strings, Base64 or ASCII recovery, invisible Unicode normalization, and alternate quoting or subexpressions. On that `realistic_v2` slice, Foundation-Sec stays correct on `42/46` strict candidates, but `invoke_expression_format_string` produces a new `4/4` miss pattern. The important mechanistic read is still that the route does not disappear. On the realistic-v2 miss slice, the early `L0H11/L0H9` ablation still flips `4/4`, the minimal direct route `L0H11 -> L12H15/H5/H4` still flips `4/4` under path patching, and the fuller late carrier `L12H15/L12H5/L12H4/L12H28` also stays strongly sufficient under path patching even though late-bundle ablation is anti-causal on both seeds and variants. For readers with standard mechanistic-interpretability background: the simplest interpretation remains a **necessity/sufficiency split**. The obfuscation shifts the final decision across the boundary without deleting the validated route.
...
| `invoke_expression_format_string` | `4` `realistic_v2` strict candidate seed/variant pairs | `4/4` misses | New realistic-benchmark failure mode; early route and late carrier remain patch-sufficient on the misses |
...
- `invoke_expression_format_string`, which produces a `4/4` miss pattern on the new `realistic_v2` slice while leaving the core patch-sufficient route intact under the current probes
